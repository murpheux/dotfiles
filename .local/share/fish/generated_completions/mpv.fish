...
complete -c mpv -l hidpi-window-scale -l no-hidpi-window-scale -d '(OS X, Windows, X11, and Wayland only) Scale the window size according to the…'
...
complete -c mpv -l cache-secs -d Deprecated
...
complete -c mpv -l wayland-app-id -d 'Set the client app id for Wayland-based video output methods'
...
complete -c mpv -l wayland-edge-pixels-touch -d 'Defines the size of an edge border (default: 64) to initiate client side resi…'
...
complete -c mpv -l cover-art-auto -d 'Whether to load _external_ cover art automatically (default: fuzzy)'
...
complete -c mpv -l vo-sixel-reqcolors -d 'Set up libsixel to use required number of colors for dynamic palette'
complete -c mpv -l vo-sixel-threshold -d 'When using a dynamic palette, defines the threshold to change the palette - a…'
